...
for fold, (train_idx, testval_idx) in enumerate(kfold.split(indices, all_labels)):
    ...
    for epoch in range(num_epochs):
        ...
        if val_epoch_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), f"mobilenet_eye_model_best_fold{fold+1}.pth")
            print(f"模型已保存为 mobilenet_eye_model_best_fold{fold+1}.pth (best)")
    ...
    # 保存最终模型
    torch.save(model.state_dict(), f"mobilenet_eye_model_fold{fold+1}.pth")
    print(f"模型已保存为 mobilenet_eye_model_fold{fold+1}.pth (last)")
    ...
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.title(f'Loss Curve Fold {fold+1} (MobileNet)')
    plt.legend()
    plt.savefig(f'mobilenet_loss_curve_fold{fold+1}.png')
    plt.close()
    ...
    plt.xlabel('Epoch')
    plt.ylabel('Accuracy')
    plt.title(f'Accuracy Curve Fold {fold+1} (MobileNet)')
    plt.legend()
    plt.savefig(f'mobilenet_acc_curve_fold{fold+1}.png')
    plt.close()

    # 测试集评估，加载最佳模型
    model.load_state_dict(torch.load(f"mobilenet_eye_model_best_fold{fold+1}.pth", map_location=device))
    model.eval()
    test_loss = 0.0
    test_corrects = 0
    test_samples = 0
